...
import FlowModel from '../../models/types/flowModel'
...
router.get("/:page", _authMiddleware, async function(req, res) {
    ...
    let flowList = await flowDb
        .find({ userid: req.loginuser.userId })
        .sort("-createdAt")
        .limit(pagelimit)
...
router.post("/newflowcomment", _authMiddleware, async function(req, res) {
        ...
        if (!flowModel)
            return res.send(JDTModel.Model(null, false, 403))
     ...
     * search
     */
router.get("/getflowcomment/:id/:page/:search?", _authMiddleware, async function(req, res) {
        console.log("flow/getflowcomment/", req.params.id, '/', req.params.page, "/", req.params.search)
        ...
        let userModelList = await userDb.find({ userId: userIdMapList, $or: where })
            .limit(pagelimit)
            .skip(req.params.page * pagelimit)
...
router.post("/likeflow", _authMiddleware, async function(req, res) {
    ...
    let likeIndex = flowModel.likes.map(e => e.userid).indexOf(req.loginuser.userId)
    if (likeIndex == -1) flowModel.likes.push({ userid: req.loginuser.userId })
    else flowModel.likes = flowModel.likes.filter(e => e.userid != req.loginuser.userId)
 ...
 * _id
 */
router.get("/likeflow/:id/:page/:search?", _authMiddleware, async function(req, res) {
    ...
    let userModelList = await userDb.find({ userId: userIdList, $or: whereQuery })
        .limit(pagelimit)
    skip(req.params.page * pagelimit)

    let resultModel = new LikeModelList(userModelList, flowModel.likes, req.loginuser)
 ...
 * search
 */
router.get("/seenflow/:id/:page/:search?", _authMiddleware, async function(req, res) {
    console.log("flow/seenflow/", req.params.id, "/", req.params.page, "/", req.params.search)
    if (req.params.search)
        req.params.search = ""
    ...
    let userList = await userDb.find({ userId: flowModel.seens, $or: whereQuery })
        .limit(pagelimit)
        .skip(req.params.page * pagelimit)
