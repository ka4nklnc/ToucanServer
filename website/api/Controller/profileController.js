...
router.get("/getuserid/:userid", _authMiddleware, async function(req, res) {
    console.log("profile/getuserid/", req.params.userid);
    let userModel = await userDb.findOne({
...
router.post("/update", _authMiddleware, async function(req, res) {
    ...
    if (
        await CheckerControl.usernameControl(
            req.body.username,
    ...
    } else return res.send(JDTModel.Model(null, false, 406));
...
router.get(
    ...
    async function(req, res) {
        ...
        let resultModel = StoryModelList(
            storyList, [userModel],
            req.loginuser.userId
        );
...
router.get(
    ...
    async function(req, res) {
        ...
        var state =
            req.loginuser.followingList.indexOf(user.userId) == -1 ? false : true;
...
router.post("/follow", _authMiddleware, async function(req, res) {
    ...
    if (indexFollowing != -1)
        req.loginuser.followingList.splice(indexFollowing, 1);
