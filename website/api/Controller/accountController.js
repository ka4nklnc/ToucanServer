...
import mail from "../Helpers/MailSenderHelper";
...
router.post("/signin", async function(req, res) {
    ...
    if (accountModel != null) {
        if (accountModel.isverification) {
            ...
            if (userModel != null) res.send(JDT.Model(userModel));
            else res.send(JDT.Model(null, false, 404));
        } else {
...
router.post("/signup", async function(req, res) {
    ...
    //Signup Formats Start
    if (FormatHelper.mailFormat(req.body.email.trim()))
        return res.send(JDTModel.Model());

    if (FormatHelper.phoneFormat(req.body.phone.trim()))
        return res.send(JDTModel.Model());
    //Signup Formats End

    //Signup Checker Start

    //Signup Checker End

    let accountEmailModel = await accountDb.findOne({
        email: req.body.email.trim(),
    });

    let accountPhoneModel = await accountDb.findOne({
    ...
    sendVerificationMail(newAccountModel);
    ...
    let newUserModel = new userDb({
        ...
        phone: req.body.phone.trim(),
        profileurl: "default",
        namesurname: req.body.namesurname.trim(),
        bio: "",
        follower: 0,
        following: 0,
        shared: 0,
...
router.get("/checkmail/:id", async function(req, res) {
    ...
    let accountModel = await accountDb.findOne({
        $or: [{ username: req.params.id.trim() }, { email: req.params.id.trim() }],
    });
    ...
    if (accountModel != null) {
        ...
        res.json(JDT.Model(null, false, 401));
    }
});
...
function sendVerificationMail(accountModel) {
    ...
    if (!accountModel.isverification) {
        accountModel.verificationcode = randomString;
        accountModel.save();
